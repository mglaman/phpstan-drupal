...
//            __DIR__ . '/../fixtures/drupal/modules/module_with_tests/tests/src/TestSite/ModuleWithTestsTestSite.php',
        ...
        foreach ($paths as $path) {
            $errors = $this->runAnalyze($path);
            // @todo the following assertions "prove" it can be autoloaded after Drupal autoloader executed.
            $this->assertTrue(class_exists('Drupal\Tests\BrowserTestBase'));
            $this->assertTrue(trait_exists('Drupal\Tests\PhpunitCompatibilityTrait'));
            $this->assertCount(0, $errors->getErrors(), implode(PHP_EOL, array_map(static function (Error $error) {
                return $error->getMessage();
            }, $errors->getErrors())));
            $this->assertCount(0, implode(PHP_EOL, $errors->getInternalErrors()));
        }
    }
